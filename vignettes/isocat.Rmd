...
The contents of these columns are passed to the function `isotopeAssignmentModel` as vectors, along with the object names of isoscape and isoscape-SD objects. The output is a SpatRaster with layers named corresponding to the individual IDs.
...
To compare multiple surfaces to one another, `isocat` includes a `surfaceSimilarityMatrix` function to create a similarity matrix of the surfaces. The output is a symmetric matrix with row and column names corresponding to the layer names of the surfaces to be compared. (The legacy `simmatrixMaker` function returns the same result for `RasterStack` inputs.)
...
For each group of individuals of common origin, create an aggregate surface of mean within-group probability of origin using the `meanAggregateClusterProbability` function. This function returns a SpatRaster with a layer for each cluster fed into it.
...
To visualize the general regions of a spatial range most associated with each aggregate surface, `isocat` includes a `projectSummaryMaxSurface` function. This function associates each cell of a spatial extent with the identity of the discrete layer with the highest relative value at that location. The output is a categorical SpatRaster that, in this context, shows the regions of highest relative association with the aggregated origins of groups of individuals. This summary surface is not appropriate for summary statistics (which we recommend be applied to mean aggregate surfaces for individuals sharing common origins, if not to the individual origin maps themselves), but does serve as a visual summary of the relative regions of probable origins of each group.
...
# Example point near the centre of the surface (coordinates are in the isoscape's
# equal-area projection, in metres).
ex_ext <- as.vector(terra::ext(p))
pt <- data.frame(x = mean(ex_ext[1:2]), y = mean(ex_ext[3:4]))
ptDeets <- list(
  geom_point(
    data = pt, 
...
ex_hist <- data.frame(x = as.vector(p[])) |>
  ggplot() +
  geom_density(aes(x = x)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(name = "Probability Value") +
  theme_bw() +
  geom_vline(aes(xintercept = terra::extract(p, cbind(pt$x, pt$y))[1, 1]), linetype = "dashed", col = "red")
...
cumsum_hist <- data.frame(x = as.vector(CumSumEx[])) |> 
  ggplot() +
  geom_density(aes(x = x)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(name = "Cumulative Sum\nProbability Value") +
  theme_bw() +
  geom_vline(aes(xintercept = terra::extract(CumSumEx, cbind(pt$x, pt$y))[1, 1]), linetype = "dashed", col = "red")
...
odds_hist <- data.frame(x = as.vector(OddsRatioEx[])) |> 
  ggplot() +
  geom_density(aes(x = x)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(name = "Odds Ratio Value") +
  theme_bw() +
  geom_vline(aes(xintercept = terra::extract(OddsRatioEx, cbind(pt$x, pt$y))[1, 1]), linetype = "dashed", col = "red")
...
quantile_hist <- data.frame(x = as.vector(QuantileEx[])) |> 
  ggplot() +
  geom_density(aes(x = x)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(name = "Quantile Value") +
  theme_bw() +
  geom_vline(aes(xintercept = terra::extract(QuantileEx, cbind(pt$x, pt$y))[1, 1]), linetype = "dashed", col = "red")
...
quantsim_hist <- data.frame(x = as.vector(QuantSimEx[])) |> 
  ggplot() +
  geom_density(aes(x = x)) +
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(name = "Quantile-Simulation Value") +
  theme_bw() +
  geom_vline(aes(xintercept = terra::extract(QuantSimEx, cbind(pt$x, pt$y))[1, 1]), linetype = "dashed", col = "red")
