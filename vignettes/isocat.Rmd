...
```{r load isoscape data,  fig.width=6, fig.height=4}
data(isoscape)
...
```{r prob of orgin surface, fig.width=6, fig.height=6}
...
```{r Create mean aggregate surfaces, eval = T}
...
```{r make cumulative sum surface, eval = T}
CumSumEx <- makecumsumSurface(p)
```


```{r plot cumulative sum surface, eval = T}
...
```{r odds ratio surface, eval = T}
OddsRatioEx <- makeOddsSurfaces(p)
```


```{r eval odds ratio surface}
...
```{r quantile surface, eval = T}
QuantileEx <- makeQuantileSurfaces(p)
```


```{r eval quantile surface}
...
```{r eval quantsim surface, eval = T}
