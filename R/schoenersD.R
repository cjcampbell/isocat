...
schoenersD <- function(rast1, rast2){
  ...
  if( global(rast1, "sum", na.rm = TRUE) != 1 ) { rast1 <- .normprodrast(rast1) }
  if( global(rast2, "sum", na.rm = TRUE) != 1 ) { rast2 <- .normprodrast(rast2) }

  1 - (0.5 * global(abs(rast1 - rast2), "sum", na.rm = TRUE))
